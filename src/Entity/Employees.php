...
class Employees
{
    ...
    #[Assert\Regex(
        pattern: "/^(9\d{7}|40\d{6}|41\d{6}|42\d{6}|44\d{6}|5[0-5]\d{6}|58\d{6}|2\d{7}|46\d{6})$/",
    ...
    private ?string $EmpCin = null;

    #[ORM\ManyToMany(targetEntity: Project::class, inversedBy: 'listEmployees')]
    private Collection $listProjects;

    public function __construct()
    {
        $this->Leaves = new ArrayCollection();
        $this->birthDate = new \DateTime('1990-01-01');
        $this->listProjects = new ArrayCollection();
    }
    ...
    public function getEmpCin(): ?string
    {
        return $this->EmpCin;
    }

    public function setEmpCin(?string $EmpCin): static
    {
        $this->EmpCin = $EmpCin;
     ...
     */
    public function getListProjects(): Collection
    {
        return $this->listProjects;
    }

    public function addListProject(Project $listProject): static
    {
        if (!$this->listProjects->contains($listProject)) {
            $this->listProjects->add($listProject);
        }

        return $this;
    }

    public function removeListProject(Project $listProject): static
    {
        $this->listProjects->removeElement($listProject);
